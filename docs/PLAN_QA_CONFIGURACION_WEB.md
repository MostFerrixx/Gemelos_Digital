...
| H-60 | **ALTO** (configuración) | QA combinadas R-06 | **"Aplicar" dejaba en `config.json` claves de la configuración anterior**: aplicar el canónico después de C-12 dejaba `personas`, `equipos` y `zonas_picking` (y `personas` manda sobre la flota): la configuración vigente no era la de la pantalla. "Run" no estaba afectado (usa la copia temporal, BK-11) | `config_manager.save_config` mezclaba la vigente con la nueva (contrato de cuando el formulario no traía los bloques que no edita; desde H-01 los trae) | **Corregido**: Aplicar REEMPLAZA; las claves quitadas se registran y la web las avisa ("Se quitaron de config.json..."). Verificado con clics reales. +1 test | **Cerrado** |
