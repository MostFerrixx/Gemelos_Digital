...
class AnalyticsExporter:
    ...
    def _append_inbound_sheet(self, excel_path):
        ...
        try:
            ...
            ws.append([])
            ws.append(['FLOTA COMPARTIDA (F5a: pick vs putaway)'])
            ws.append(['Prioridad configurada', ib.get('putaway_priority')])
            ws.append(['Espera pallet->agente promedio (s)', ib.get('avg_putaway_wait')])
            ws.append(['Espera pallet->agente maxima (s)', ib.get('max_putaway_wait')])
            ws.append([])
            ws.append(['CROSS-DOCKING (F5b: stock del dia rescata backorders)'])
            ws.append(['Activo', 'SI' if ib.get('cross_dock_enabled') else 'NO'])
            ws.append(['Unidades rescatadas', ib.get('cross_dock_units_rescued', 0)])
            ws.append(['Picks dinamicos creados', ib.get('cross_dock_picks_created', 0)])
            wb.save(excel_path)
